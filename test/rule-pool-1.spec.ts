...
import {myRule1} from "./my-rule-1";
import {reflectionPool} from "@leyyo/core";
import {TmpClass, TmpField, TmpField2, TmpMethod, TmpParam} from "./decorators";
...
describe('RulePool', () => {
    ...
    const prop_3 = clazz.getInstanceProperty('myProperty3');
    ...
    it('- prop_2 Typed', () => {
        assert.equal(prop_2.type, Object);
    });

    it('- prop_3 Ruled', () => {
        assert.equal(prop_3.hasDecorator(TmpField), false);
    });
    it('- prop_3 Value', () => {
        assert.equal(prop_3.getValueByDeco(TmpField), undefined);
    });
    it('- prop_3 Typed', () => {
        assert.equal(prop_3.type, Object);
    });
